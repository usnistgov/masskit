...
from massspec.data_specs.schemas import experimental_fields
from massspec.peptide.encoding import h2o_mass
from massspec.spectrum.ipython import is_notebook
import re
...
from massspec.utils.fingerprints import SpectrumTanimotoFingerPrint
from abc import ABC, abstractmethod
from massspec.spectrum.plotting import spectrum_plot
...
class Ions(ABC):
    ...
    def __init__(
        ...
        self.jitter = jitter
        self.starts = None  # used for high resolution spectra, mz bin start
        self.stops = None  # used for high resolution spectra, mz bin end
        self.join = None
        return
    ...
    @abstractmethod
    def intersect(self, comparison_ions):
        pass
    ...
    def clear_and_intersect(self, ion2, index1, index2):
        """_
        ...
        if index1 is None or index2 is None:
            ...
            common_mz, index1, index2 = ion1.intersect(ion2)
        else:
            ion1 = self
    ...
    def cosine_score(
        ...
        ion1, ion2, index1, index2 = self.clear_and_intersect(ion2, index1, index2)
    ...
    def composite_score(
        ...
        """
        if index1 is None or index2 is None:
            common_mz, index1, index2 = self.intersect(ion2)
        ...
        common_mz_diag, index1_diag, index2_diag = self.intersect(diag_poisson.products)
...
class HiResIons(Ions):
    ...
    def intersect(self, comparison_ions):
        ...
        :return: mz values of matches, matched peak indexes in self, matched peak indexes in comparison_ions
        """
        return intersect_hires(
            self.starts,
            self.stops,
            self.mz,
...
#@jit(nopython=True, parallel=True, cache=True)
def intersect_hires(ions1_starts, ions1_stops, ions1_mz, ions2_starts, ions2_stops):
    """
    ...
    :param ions1_mz: mz positions of the first ion series to compare
    ...
    :return: mz values of matches, matched peak indexes in ions1, matched peak indexes in ion2
    """
    ...
    common_mz = np.empty((size,), dtype=np.float64)
    ...
    for i in range(len(start_indx)):
        ...
        common_mz[position: position + num_slots] = ions1_mz[start_indx[i]]
        position += num_slots

    return common_mz, index1, index2
...
class BaseSpectrum:
    ...
    def to_msp(self):
        ...
        for i in range(num_peaks):
            ret_value += f"{self.products.mz[i]:.15g}\t{self.products.intensity[i]:.15g}"
    ...
    def copy_annot(self, spectrum2):
        ...
        """
        common_mz, index1, index2 = self.products.intersect(spectrum2.products)
    ...
    def intersect(self, spectrum2):
        """
        intersect product ions with another spectrum's product ions

        :param spectrum2: comparison spectrum
        :return: common_mz values, matched ions indices in this spectrum, matched ion indices in comparison spectrum
        """
        return self.products.intersect(spectrum2.products)
    ...
    def __str__(self):
        if is_notebook():
            ...
            return \
                f'<img data-content="massspec/spectrum" src="data:image/png;base64,{val}" alt="spectrum {self.name}"/>'
        else:
...
def init_spectrum(
    ...
    if precursor_mass_info is None:
        precursor_mass_info = MassInfo(10.0, "ppm", "monoisotopic", "", 1)
    if product_mass_info is None:
