...
import pandas as pd
from massspec.data_specs.schemas import ion_annot_fields
...
class Join(ABC):
    ...
    def join_2_spectra(spectra1, spectra2, tiebreaker="mz"):
        ...
        join_1_2 = []
        join_2_1 = []
        mz, index1, index2 = spectra1.intersect(spectra2)
        for exp_peak_index in range(len(spectra1.products.mz)):
            pos = np.argwhere(index1 == exp_peak_index)
            if pos.shape[0] != 0:
                if tiebreaker == "mz":
                    first = True
                    for i in range(pos.shape[0]):
                        dmz = abs(spectra1.products.mz[exp_peak_index] - spectra2.products.mz[index2[pos[i, 0]]])
                    ...
                    first = True
                    for i in range(pos.shape[0]):
                        dintensity = abs(spectra1.products.intensity[exp_peak_index] - spectra2.products.intensity[index2[pos[i, 0]]])
...
class ThreewayJoin(Join):
    ...
    def do_join(self, tiebreaker="mz"):
        ...
        for i in range(len(self.exp_lib_map)):
            ...
            experimental_list_id.extend([self.exp_lib_map.get_ids()[i]] * len(result[0]))
            predicted_list_id.extend([self.pred_lib_map.get_ids()[i]] * len(result[1]))
            theoretical_list_id.extend([self.theo_lib_map.get_ids()[i]] * len(result[2]))
