...
class BaseMLConfig(BaseConfig):
    ...
    data_path = str(Path.home() / "data" / "deep" / "spectra" / "ei")
...
class ModelParams:
    ...
    """
    debug = 0  # general purpose int for debugging purposes
...
class SmallMolMLConfig(BaseMLConfig):
    ...
    tani_and_cos = False  # combine the tanimoto and cosine score
...
class BaseMSConfig(BaseConfig):
    ...
    """
    down_shift = 0.0  # downshift mz values by this amount when filling out spectral arrays
    max_mz = 1000.0  # the maximum mz value
...
class TandemMLConfig(TandemConfig, ModelParams, SmallMolMLConfig):
    ...
    num_search_results = 0   # number of search results to use for training. 0 = all
    ...
    mz_delta = False  # create channel containing diff of query mz minus hit mz
    precursor_minus_mz_channel = False  # create channel containing precursor-mz.  normalized by max_mz
    num_beginning_layers = 0  # number of beginning layers
    ...
    def get_input_shape(self, num_spectra=1):
        ...
        num_channels = 1 + self.energy_channel + self.mz_channel + self.precursor_minus_mz_channel + self.mz_delta
