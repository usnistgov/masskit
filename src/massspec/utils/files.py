...
import massspec.small_molecule
...
import json
from massspec.constants import SET_NAMES, EPSILON
from massspec.data_specs.schemas import spectrums_schema, molecules_schema, set_field_int_metadata, \
    mod_names_field, hitlist_schema
from massspec.peptide.encoding import mod_masses
from massspec.small_molecule import threed, utils
...
try:
    ...
    pass
from massspec.utils.fingerprints import ECFPFingerprint
...
def spectra_to_array(spectra, min_intensity=0, max_mz=2000):
    """
...
def spectra_to_mgf(fp, spectra):
    ...
    for i in range(len(spectra)):
        ...
        if spectra[i].peptide is not None:
            print(f"SEQ={spectra[i].peptide}", file=fp)
...
def spectra_to_mzxml(fp, spectra, mzxml_attributes=None, min_intensity=EPSILON, compress=True, use_id_as_scan=True):
    ...
    info = {'startTime': 0, 'endTime': 86400, 'scanCount': 0, 'fileName': 'unknown', 'fileType': 'RAWData',
    ...
    scan_number = 1
    for i in range(len(spectra)):
        spectrum = spectra[i].filter(min_mz=1.0, min_intensity=min_intensity)
        ...
        if spectrum.products is not None and len(spectrum.products.mz) > 0:
            ...
            retentionTime = spectrum.retention_time if spectrum.retention_time is not None else 0.0
            ...
            print(f'          retentionTime="PT{retentionTime:.6f}S"', file=fp)
            if collisionEnergy != "":
                print(f'          collisionEnergy="{collisionEnergy:.6f}"', file=fp)
            print(f'          lowMz="{min(spectrum.products.mz):.6f}"', file=fp)
            print(f'          highMz="{max(spectrum.products.mz):.6f}"', file=fp)
            basePeak = np.argmax(spectrum.products.intensity)
            print(f'          basePeakMz="{spectrum.products.mz[basePeak]:.6f}"', file=fp)
            print(f'          basePeakIntensity="{spectrum.products.intensity[basePeak]:.6f}"', file=fp)
            print(f'          totIonCurrent="{spectrum.products.intensity.sum():.6f}"', file=fp)
            ...
            print(f'      <precursorMz precursorScanNum="{scan}" precursorIntensity="{precursorIntensity:.6f}"'
                  f' precursorCharge="{precursorCharge}" activationMethod="{activationMethod}">{precursorMz:.6f}</precursorMz>', file=fp)
...
def load_msp2array(
    ...
    while line:
            ...
            if (
                ...
                if len(values) == 2:
                    ...
                    if values[0] == "Name":
                        ...
                        if match:
                            row["nce"] = int(match.group(1))
                        match = re.search(r"(\d+)eV", values[1])
                        if match:
                            row["ev"] = int(match.group(1))
                        ...
                        for match in re.finditer(
                            ...
                            if match.group(1) == "Parent":
                                ...
                                # row["mod_names"] = [x[1] for x in submatch]
        ...
        if len(mz) != 0:
            row['mz'] = mz
            row['intensity'] = intensity
            spectrum = init_spectrum(mz, intensity, row={})
            spectrum.charge = row.get('charge', None)
            spectrum.peptide = row.get('peptide', None)
            spectrum.peptide_len = row.get('peptide_len', None)
            spectrum.mod_names = row.get('mod_names', None)
...
def load_sdf2array(
    ...
    for i, mol in enumerate(Chem.SDMolSupplier(filename, sanitize=False)):
        ...
        try:
            mol = massspec.small_molecule.utils.standardize_mol(mol)
...
class MzTab_Reader():
    ...
    """
    def __init__(self, fp, dedup=False):
        self.dedup = dedup
        self.metadata_rows = []
        self.psm_rows = []
    ...
    def parse_psm_row(self, row):
        ...
        mods = row["modifications"].split('|')[0].split(',')  # vbar is to get rid of extra stuff added by mascot
        for mod in mods:
