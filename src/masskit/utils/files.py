...
from ..data_specs import file_schemas as mkfile_schemas
from ..data_specs import schemas as mkschemas
from ..small_molecule import threed as mkthreed
from ..small_molecule import utils as mksmutils
...
def write_parquet(fp, table):
    ...
    """
    fp = mkgeneral.open_if_filename(fp, 'wb')
...
def read_parquet(fp, columns=None, num=None, filters=None):
    ...
    """
    fp = mkgeneral.open_if_filename(fp, 'rb')
...
def spectra_to_array(spectra,
    ...
    records_schema = mkfile_schemas.schema_groups[schema_group]['flat_schema']
...
class BatchLoader:
    def __init__(self, file_type, row_batch_size=5000, format=None, num=None):
        ...
        self.schema_group = mkfile_schemas.schema_groups[self.format['schema_group']]
    ...
    ecfp4 = mkfingerprints.ECFPFingerprint()
    ...
    def mol2row(cls, mol, max_size:int=0, skip_computed_props=True, skip_expensive:bool=True) -> Dict:
        ...
        try:
            mol = mksmutils.standardize_mol(mol)
        ...
        try:
            ...
            if not skip_computed_props:
                ...
                if not skip_expensive:
                    mol, conformer_ids, return_value = mkthreed.create_conformer(mol)
                    if return_value == -1:
                    ...
                    bounding_box = mkthreed.bounding_box(mol)
                ...
                new_row["num_undef_double"] = len(mksmutils.get_unspec_double_bonds(mol))
    ...
    def load(self, fp):
        ...
        while line:
                            ...
                            for match in re.finditer(
                                ...
                                if match.group(1) == "Parent":
                                    ...
                                    if not submatch:
                                        ...
                                        submatch = re.findall(r'/(\d+),[A-Z]+,([^/]*)', match.group(2))
                                    row["mod_names"] = [mkdata.mod_masses.dictionary.index(nist_mod_2_unimod.get(x[1], x[1])) for x in submatch]
            ...
            if len(mz) != 0:
                ...
                spectrum = mkspectra.Spectrum(mz=mz, intensity=intensity, row={}, precursor_mz=row['precursor_mz'],
...
class MGFLoader(BatchLoader):   
    ...
    def load(self, fp):
        ...
        for line in fp:
            ...
            row = {
                "id": self.current_id,
                "set": np.random.choice(mkdata.SET_NAMES, p=self.set_probabilities),
            }
            ...
            if len(mz) != 0:      
                row['mz'] = mz
                row['intensity'] = intensity
                spectrum = mkspectra.Spectrum(mz=mz, intensity=intensity, row=row, precursor_mz=row['precursor_mz'],
                                        precursor_intensity=row.get('precursor_intensity', None))
                spectrum.charge = row.get('charge', None)
                spectrum.peptide = row.get('peptide', None)
                spectrum.peptide_len = row.get('peptide_len', None)
    ...
    def load(self, fp):
        ...
        while True:
            ...
            if self.format['source'] == "nist":
                ...
                spectrum = mkspectra.Spectrum(product_mass_info=product_mass_info,
        ...
        for next_chunk in fp:
            ...
            for i in range(len(table)):
                ...
                if self.current_id is not None:
                    ...
                    self.current_id += 1
            table = table.append_column('mol', pa.array(mols, type=mkarrow_types.MolArrowType()))
            if self.current_id is not None:
...
def records2table(records, schema_group):
    ...
    if has_spectra:
        structarray = mktables.table_to_structarray(table, mkarrow_types.SpectrumArrowType)
    table = table.select([x for x in table.column_names if x in schema_group['nested_schema'].names])
    if has_spectra:
        table = mktables.table_add_structarray(table, structarray)
    return table
...
def create_table_with_mod_dict(records, schema):
    ...
    mod_names = pa.DictionaryArray.from_arrays(indices=pa.array(records['mod_names'], type=pa.list_(pa.int16())), dictionary=mkdata.mod_masses.dictionary)
    del records['mod_names']
    table = pa.table(records, schema)
    table.append_column(mkschemas.mod_names_field, mod_names)
    return table
...
class MzTab_Reader():
    ...
    def read_sections(self, fp):
        fp = mkgeneral.open_if_filename(fp, 'r')
        for line in fp:
    ...
    def parse_psm(self):
        ...
        records = empty_records(mkschemas.hitlist_schema)
        prev_hit_id = None
        for row in reader:
        ...
        table = pa.table(records, mkschemas.hitlist_schema)
        self.psm = table
    ...
    def get_hitlist(self):
        ...
        return mkhitlist.Hitlist(df.set_index(['query_id','hit_id']))
...
class BatchFileReader:
    def __init__(self, 
        ...
        if format['format'] == 'parquet':
            ...
            self.dataset = mkgeneral.open_if_filename(filename, mode="r")
            ...
            self.dataset =  pacsv.open_csv(filename, read_options=read_options, parse_options=parse_options)
             ...
             # supports gz and bz2 compression
            self.dataset = mkgeneral.open_if_filename(filename, mode="rb")
...
def spectrum2msp(spectrum, annotate=False):
    output = StringIO()
    mkspectrum_writers.spectra_to_msp(output, [spectrum], annotate_peptide=annotate)
    return output.getvalue() 

def spectrum2mgf(spectrum):
    output = StringIO()
    mkspectrum_writers.spectra_to_mgf(output, [spectrum])
    return output.getvalue() 
