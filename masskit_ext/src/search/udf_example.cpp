...
arrow::Status SampleFunction(cp::KernelContext* ctx, 
  ...
  //std::shared_ptr<arrow::Scalar> n_scalar = batch[3].scalar;
  //std::shared_ptr<arrow::Int64Scalar> n_scalar_int = std::dynamic_pointer_cast<arrow::Int64Scalar>(n_scalar);
  auto z_scalar = batch[2].scalar;
...
arrow::Status Execute() {
  ...
  //auto func = std::make_shared<cp::ScalarFunction>(name, cp::Arity::VarArgs(), func_doc);
  ...
  std::cout << "Y" << std::endl << y->ToString() << std::endl;
  //std::cout << "Z" << std::endl << z->ToString() << std::endl;
  //arrow::Int64Scalar z(10);

  cp::ExecBatch batch({arrow::Datum(x), arrow::Datum(y), 
                       arrow::Datum(std::make_shared<arrow::Int64Scalar>(10))}, x->length());

  // ARROW_ASSIGN_OR_RAISE(auto res, cp::CallFunction(name, {x, y, z}));
  ARROW_ASSIGN_OR_RAISE(auto res, cp::CallFunction(name, batch));
