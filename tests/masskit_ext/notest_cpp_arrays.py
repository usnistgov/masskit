...
print(masskit_ext.arrow_chunk(table.column('a')))
print(masskit_ext.table_info(table))

s = masskit_ext.BruteForceIndex("george")
...
for i in range(a.num_chunks):
    result.append(masskit_ext.arrow_add(a.chunk(i), b.chunk(i), c.chunk(i)))
...
result = masskit_ext.numpy_add(table.column('a').to_numpy(), table.column('b').to_numpy())
