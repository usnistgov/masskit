...
def test_intersect_hires_spectrum(hi_res1: masskit.spectra.Spectrum, hi_res2: masskit.spectra.Spectrum):
...
def test_copy_ions(hi_res1: masskit.spectra.Spectrum):
...
def test_filter_ions(hi_res1: masskit.spectra.Spectrum):
...
def test_normalize(hi_res1: masskit.spectra.Spectrum):
...
def test_merge(hi_res1: masskit.spectra.Spectrum, hi_res2: masskit.spectra.Spectrum):
...
def test_evenly_space(hi_res1: masskit.spectra.Spectrum):
...
def test_evenly_space_stddev(hi_res3: masskit.spectra.Spectrum):
...
def test_cosine_score_tiebreak(spectrum2: masskit.spectra.Spectrum, predicted_spectrum1: masskit.spectra.Spectrum):
    # according to mspepsearch should be 549.  spectra 484, first one in Qian's plot
    score = predicted_spectrum1.cosine_score(spectrum2, tiebreaker='mz')
    assert score == approx(992.2658666598618)

def test_to_msp(hi_res1: masskit.spectra.Spectrum):
    ret_value = hi_res1.to_msp()
    len(ret_value) > 10

def test_accumulator(hi_res1: masskit.spectra.Spectrum, hi_res2: masskit.spectra.Spectrum):
    acc = masskit.spectra.AccumulatorSpectrum(mz=np.linspace(0.1, 2000, 20000), tolerance=0.05)
...
def test_shift_mz(hi_res1: masskit.spectra.Spectrum):
...
def test_windowed_filter(predicted_spectrum1: masskit.spectra.Spectrum):
...
def test_parent_filter(hi_res3: masskit.spectra.Spectrum):
...
def test_evenly_space_cosine_score(hi_res3: masskit.spectra.Spectrum):
    initial_spectrum = hi_res3.copy()  # make a copy
...
def test_single_match(hi_res3: masskit.spectra.Spectrum):
